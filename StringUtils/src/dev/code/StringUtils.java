...
public class StringUtils {
	...
	public static boolean isPalindrome(String s) {
		...
		for (int i = 0; i < newStr.length(); i++) {
			...
			if (chrF != chrB) {
				 ...
				 * chrF + " chrB " + chrB); System.out.println("Err2 " + s +
				 * " is not a palindrome!  Try again...");
				 */
				isPalindrome = false;
				break;
			}
			isPalindrome = true; // will be set to false if not true so okay to
									// always set here
	...
	public static String checkParens(String s) {
		String result = "";
		char chr = '\0';
		int l = 0, r = 0;

		for (int i = 0; i < s.length(); i++) {
			chr = StringUtils.getChar(s, i);
			switch (chr) {
			case '(':
				l++;
				break;
		...
		if (r != l) {
				result = "ERROR!!! Right parens are: " + r + ".  Left parens are: " + l;
			} else {
				result = "Correct! Right parens are: " + r + ".  Left parens are: " + l;
			}
		return result;
	}
}
